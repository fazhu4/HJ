...
public interface MyNotesMapper {
    ...
    //获取我的笔记
    @Select("select `note_name`,`img`,`mnid` from `my_notes` where `uid` = #{uid}")
    List<MyNotes> getMyNotes(@Param("uid") int uid);
    //删除我的笔记
    @Delete("delete from `my_notes` where `CNid` = #{CNid} and `uid` = #{uid}")
    ...
    //获取我喜欢的笔记id
    @Select("select `CNid` form `like_note` where `uid` = #{uid}")
